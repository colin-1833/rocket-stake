...
const apply_lifecycles = (runtime: Runtime, func: () => Promise<void>): Promise<void> => new Promise(async (resolve, reject) => { try {
  const { task } = runtime;
...
const main_contract_call = (runtime: Runtime, method: string, args: Array<any>, overrides?: any): Promise<string> => new Promise(async (resolve, reject) => { try {
  const {
    hardhat,
    account,
    ethereum,
